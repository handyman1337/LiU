...
def PlotErrorGraphs(ErrTrain, ErrTest, hyperparameters):
    ErrTestMinVal = np.min(ErrTest)
    ErrTestMinIdx = np.argmin(ErrTest)
    N = len(ErrTrain)
    
    plt.figure(figsize=(12,5))
    plt.title(f'Results for {hyperparameters["nTrain"]} training images, {hyperparameters["nTest"]} test images, {hyperparameters["nFeatures"]} Haar features')
...
def PlotSolvayHeatmap(cSolvay):
    ...
    # Filter heatmap for better results
    kernel = np.array([0.01, 0.03, 0.07, 0.12, 0.18, 0.20, 0.18, 0.12, 0.07, 0.03, 0.01])
    hm = np.apply_along_axis(lambda x: np.convolve(x, kernel, mode='same'), 0, hm)
    hm = np.apply_along_axis(lambda x: np.convolve(x, kernel, mode='same'), 1, hm)
